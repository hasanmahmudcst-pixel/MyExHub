...
def profile():
    ...
    conn.close()
    return render_template_string(STYLE + NAV + """<div class="container"><h2>My products</h2><div style="display:flex; flex-wrap:wrap; gap:20px; justify-content:center;">{% for i in my_items %}<div class="card" style="width:250px;"><h3>{{i[1]}}</h3><p>Sta: <b>{{i[8]}}</b></p>{% if i[7] == 0 %}<a href="/promote/{{i[0]}}" class="btn" style="background:#d1006c;">Promote (1$)</a>{% else %}<p style="color:green;">⭐ Promoted</p>{% endif %}</div>{% endfor %}</div></div>""" + FOOTER, my_items=my_items)

@app.route('/promote/<int:pid>', methods=['GET', 'POST'])
def promote(pid):
    if 'user' not in session: return redirect(url_for('login'))
    if request.method == 'POST':
        conn = sqlite3.connect('database.db')
        conn.execute("INSERT INTO payments (user_email, product_id, trxid, method) VALUES (?,?,?,?)", (session['user'], pid, request.form.get('trx'), request.form.get('method')))
        conn.commit(); conn.close()
        return "Payment information has been submitted. Once verified,your product will be featured."
    return render_template_string(STYLE + NAV + """<div class="card"><h2>Please promote your product</h2><p>bkah/Nagad (Send Money): <b>01712572371</b><br>Amount: <b>1$</b></p><form method="POST"><select name="method"><option>Bkash</option><option>Nagad</option></select><input name="trx" placeholder="Give me the TrxID" required><button class="btn">Verify the payment</button></form></div>""" + FOOTER)
...
def admin():
    ...
    <div class="card"><h3>পেন্ডিং পেমেন্ট (Featured Request)</h3><table><tr><th>পণ্য</th><th>TrxID</th><th>অ্যাকশন</th></tr>{% for py in pending_pay %}<tr><td>{{py[4]}}</td><td>{{py[3]}}</td><td><a href="/approve_pay/{{py[2]}}/{{py[0]}}">অ্যাপ্রুভ</a></td></tr>{% endfor %}</table></div>
...
@app.route('/approve_pay/<int:pid>/<int:payid>')
def approve_pay(pid, payid):
    if session.get('role') == 'admin':
        conn = sqlite3.connect('database.db')
        conn.execute("UPDATE products SET featured=1 WHERE id=?", (pid,))
        conn.execute("UPDATE payments SET status='Approved' WHERE id=?", (payid,))
        conn.commit(); conn.close()
    return redirect(url_for('admin'))

# --- আগের অন্যান্য ফাংশনগুলো ---
...
def login():
    if request.method == 'POST':
        conn = sqlite3.connect('database.db'); user = conn.execute("SELECT * FROM users WHERE email=?", (request.form.get('e'),)).fetchone(); conn.close()
...
@app.route('/about')
def about():
    conn = sqlite3.connect('database.db'); s = conn.execute("SELECT * FROM site_info WHERE id=1").fetchone(); conn.close()
    return render_template_string(STYLE + NAV + f"<div class='card'><h2>About</h2><p>{s[1]}</p></div>" + FOOTER)

@app.route('/contact')
def contact():
    conn = sqlite3.connect('database.db'); s = conn.execute("SELECT * FROM site_info WHERE id=1").fetchone(); conn.close()
    return render_template_string(STYLE + NAV + f"<div class='card'><h2>Contact</h2><p>Address: {s[4]}<br>Mobile: {s[2]}</p></div>" + FOOTER)

@app.route('/refund')
def refund():
    conn = sqlite3.connect('database.db'); s = conn.execute("SELECT * FROM site_info WHERE id=1").fetchone(); conn.close()
    return render_template_string(STYLE + NAV + f"<div class='card'><h2>Refund</h2><p>{s[3]}</p></div>" + FOOTER)
